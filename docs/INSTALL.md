...
通过可信的仓库 Release 页面取得 `fm10k-controlpanel-0.2.4.tar.gz`、对应 `.deb`、`release-manifest.json` 和 `SHA256SUMS`。私有仓库可在工作站使用已授权的 GitHub CLI 下载；不用把工作站的凭据复制给设备。草稿仅供维护者审核，设备在线更新不会选中草稿。

```sh
sha256sum -c SHA256SUMS
tar -xzf fm10k-controlpanel-0.2.4.tar.gz
cd fm10k-controlpanel-0.2.4
